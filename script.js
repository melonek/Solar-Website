...
// Promise‑based image preloader (for critical assets such as hero images)
...
// Simple preloader for sub‑page assets (logs success/failure)
function preloadImagesSimple(urls) {
  urls.forEach(url => {
    const img = new Image();
    img.src = url;
    img.onload = () => {
      console.log(`Preloaded image: ${url} loaded okay.`);
    };
    img.onerror = () => {
...
window.addEventListener('load', () => {
  preloadImagesSimple(subPageImages);
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Since the Services dropdown is now plain text (using a .dropbtn element),
  // we no longer modify its href. (Other dropdowns, if links, can be left alone.)
  // Remove any href manipulation.
  
  // Instead, add click event listeners to plain text dropdown buttons.
  ...
  // For other dropdowns that are still links, you can keep their behavior:
